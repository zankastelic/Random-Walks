...
S 'pomočjo' grafa se lahko prepričamo, da zgornja formula $P(S_t = b)$ drži. Če je $p =\frac{1}{2}, -a = -25, b=25$ je verjetnost enaka $0.5$. Simulirana vrednost pa znaša $0.5063$. 
...
spodnja_meja2 <- -4
zgornja_meja2 <- 36
a <- sim_sprehod(p2,spodnja_meja2,zgornja_meja2)
O <- 100
seznam2 <- replicate(O, sim_sprehod(p2, spodnja_meja2, zgornja_meja2))
max_length2 <- max(sapply(seznam2, length))
# make plot
plot(c(1, max_length2), c(spodnja_meja2, zgornja_meja2), type = "n",
     main="Random walks", xlab="cas oz. stevilo korakov", ylab="vrednost")
for (i in 1:O) {
  lines(1:length(seznam2[[i]]), seznam2[[i]],
        col = colorPicker(seznam2[[i]], max_length2), lwd = 0.5)
}
abline(h = 0, lty = "dashed")
abline(h = spodnja_meja2, lwd = 2)
abline(h = zgornja_meja2, lwd = 2)

```

V konkretnem primeru je bila zgornja meja dosežena kar 9875-krat (PREVERI). Kolikokrat in v koliko korakih je bila meja dosežena, si bomo pogledali v naslednjem razdelku. Poglejmo si še verjetnost, da pri danem $p$-ju prej dosežemo zgornjo mejo. Formula nam da
\[
...
```{r, echo=FALSE}
#verjetnost_da_prej_dosezemo_zgornjo_mejo(seznam2)
#narišimo verjetnost 

# stevilo simulacij: 
#x <- c(1:O)
#y <- c()
#for (i in seq(3, length(seznam2), by = 3)) {
#  b <- seznam[1:i]
#  p <- verjetnost_da_prej_dosezemo_zgornjo_mejo(b)
#  y <- c(y,p)
#}
...
#plot(x,y,
#     type="l", lwd=1, xlab="Število simulacij", ylab="Verjetnost")
#abline(h = abs(spodnja_meja2)/(abs(spodnja_meja2)+zgornja_meja2), lty = "dashed")
#
#stevilo_korakov_sim <- prestej(seznam2, spodnja_meja2, zgornja_meja2)
```

S pomočjo simulacije dobimo $0.9875$ kar je zelo dober približek. Iz vseh grafikonov je več kot očitno, da je stanje $0$ povrnljivo - prav tako so vsa stanja povrnljiva. 
...
Nesimetričen sprehod je simuliran tako, da si izberemo parametra p_gor in p_desno. V tem primeru smo indiferentni medtem ali gremo desno/levo ali gor/dol. Kakšne obravnave in analize so za nesimetrične sprehode nesmiselne.
...
cas_vrnitve_ladder_graf <- function(n){
  ...
  # zacnemo v 1 
  pot <- c(1)
  kam <- sample(which(sosedi[1,] %in% 1),1)
  pot <- c(pot,kam)
  while (kam != 1) {
...
izracunaj_za_vec <- function(stevilo_n,stevilo_simulacij){
  vsota <- 0
  for (j in stevilo_n){
    for (i in 1:stevilo_simulacij){
      qwer <- cas_vrnitve_ladder_graf(j)
      vsota <- vsota + qwer
    }
    vsota <- vsota / stevilo_simulacij
    cas_vrnitve_ladder <- append(cas_vrnitve_ladder,vsota)
  }
  return(cas_vrnitve_ladder)
}

#mogoče mal traja
cas_vrnitve_ladder <- izracunaj_za_vec(stevilo_n,10000)

df9 <- data.frame(stevilo_n, stevilo_simulacij, cas_vrnitve_ladder)
...
cas_pokritja_ladder <- c()
```
Zgoraj vidimo tabelo pričakovanih časov vrnitve lestvenih grafov za $n=1,2,3,4,5,6$. Če je $n=2$ je lestveni graf enak kvadratu. V tem primeru smo privzeli, da začnemo v enem izmed oglišč, ki ima $2$ soseda in ne v enem izmed notranjih, ki ima $3$ sosede